...
const SECONDS_PER_SIM_HOUR = 5; // real seconds per simulated hour -- slower so movement/paths are easy to watch
...
popRange.addEventListener(
  "input",
  () => (document.getElementById("pop-val").textContent = popRange.value),
);
...
document.getElementById("btn-generate").addEventListener("click", async () => {
  ...
  setSetupBusy(
    true,
    "Looking up the real map, then drafting citizens... (can take ~20-30s)",
  );
  hideSetupError();
  try {
    ...
    if (!res.ok)
      throw new Error((await res.json()).detail || "World generation failed");
...
function setSetupBusy(busy, msg) {
  ...
  document.getElementById("setup-status").style.display = busy
    ? "flex"
    : "none";
...
function renderTownMeta() {
  ...
  document.getElementById("town-sub").textContent =
    `Pop. ${state.citizens.length} \u00b7 ${state.world.weather} \u00b7 Economy: ${state.world.economy}`;
...
function initMap() {
  ...
  const center = state.center
    ...
    : [12.990833, 76.11495];
  map = L.map("town-map", { zoomControl: true, scrollWheelZoom: true }).setView(
    center,
    16,
  );
...
function occupationEmoji(occupation) {
  ...
  if (o.includes("teach") || o.includes("professor") || o.includes("lecturer"))
    return "\u{1F469}\u200D\u{1F3EB}";
  if (o.includes("doctor") || o.includes("physician") || o.includes("surgeon"))
    return "\u{1F469}\u200D\u2695\uFE0F";
  if (o.includes("nurse")) return "\u{1F9D1}\u200D\u2695\uFE0F";
  if (o.includes("police") || o.includes("officer") || o.includes("cop"))
    return "\u{1F46E}";
  ...
  if (
    ...
    return "\u{1F9D1}\u200D\u{1F4BC}";
  if (o.includes("engineer") || o.includes("technician"))
    return "\u{1F468}\u200D\u{1F527}";
  if (o.includes("lawyer") || o.includes("advocate"))
    return "\u{1F9D1}\u200D\u2696\uFE0F";
  if (o.includes("chef") || o.includes("cook"))
    return "\u{1F9D1}\u200D\u{1F373}";
  if (o.includes("student")) return "\u{1F9D1}\u200D\u{1F393}";
  if (o.includes("artist") || o.includes("painter") || o.includes("musician"))
    return "\u{1F9D1}\u200D\u{1F3A8}";
  if (o.includes("driver") || o.includes("pilot") || o.includes("conductor"))
    return "\u{1F9D1}\u200D\u2708\uFE0F";
  return "\u{1F9D1}";
}
...
function currentLocationFor(c) {
  ...
  const entry =
    c.schedule.find((s) => simHour >= s.start_hour && simHour < s.end_hour) ||
    c.schedule[c.schedule.length - 1];
...
async function advanceDay() {
  ...
  try {
    ...
    if (!res.ok)
      throw new Error((await res.json()).detail || "Day planning failed");
    state = await res.json();
    renderTownMeta();
    renderLog();
    clearTrails();
...
async function triggerEvent() {
  ...
  setControlBusy(
    true,
    description
      ? "Simulating that event across the town..."
      : "An event is unfolding \u2014 agencies are responding...",
  );
  try {
    ...
    if (!res.ok)
      throw new Error((await res.json()).detail || "Event simulation failed");
    state = await res.json();
    renderLog();
    renderCitizenDots();
    renderEventReport();
...
function renderEventReport() {
  ...
  const chainsHtml = ev.agent_reasoning
    .map(
      (a) =>
  ...
  const rowsHtml = ev.citizen_report
    .map(
      (r) => `
    <div class="event-row">
...
function setControlBusy(busy, msg) {
  ...
  document.getElementById("control-status").style.display = busy
    ? "flex"
    : "none";
...
function selectCitizen(id) {
  ...
  if (prev !== null && citizenMarkers[prev])
    updateCitizenIcon(
      citizenMarkers[prev],
      state.citizens.find((x) => x.id === prev),
    );
  if (citizenMarkers[id])
    updateCitizenIcon(
      citizenMarkers[id],
      state.citizens.find((x) => x.id === id),
    );
  renderDetail();
}
...
function renderDetail() {
  ...
  if (!c) {
    panel.innerHTML =
      '<div class="empty-note">Click a citizen or a building on the map to see more.</div>';
    return;
  }
  ...
  const memoriesHtml =
    (c.memories || [])
      .slice()
      .reverse()
      .map((m) => `<div class="memory-item">${escapeHtml(m)}</div>`)
      .join("") || '<p class="empty-note">No memories yet.</p>';

  const chatHtml = (c.chat_history || [])
    .map(
      (m) =>
  ...
  document
    ...
    .addEventListener("click", () => sendChat(c.id));
  document.getElementById("chat-input").addEventListener("keydown", (e) => {
    if (e.key === "Enter") sendChat(c.id);
  });
}
...
function renderBuildingDetail(panel) {
  ...
  const presentNow = state.citizens.filter(
    (c) => currentLocationFor(c) === b.name,
  );
  ...
  const listOrNone = (arr) =>
    arr.length
      ? arr
          .map(
            (c) =>
              `<div class="memory-item">${escapeHtml(c.name)} (${escapeHtml(c.occupation)})</div>`,
          )
          .join("")
...
async function sendChat(cid) {
  ...
  btn.textContent = "...";
  try {
    const res = await fetch(`${API}/citizen/${cid}/ask`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
